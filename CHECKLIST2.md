...
* [x] 行読み込み・コメント除去・空行スキップ
  - 完了条件: `#` コメントと空行が正しく無視される
* [x] トークン化（ニーモニック / レジスタ / 即値）
  - 完了条件: `LDI r1, 72` が `['LDI', 'r1', '72']` に分解される
* [x] ラベル解析（1 パス目でアドレス収集）
  - 完了条件: `loop:` 形式のラベルが認識される
* [x] 命令エンコード（NOP / HALT / LDI / OUT の 4 命令）
  - 完了条件: 各命令が 32-bit LE バイナリに正しく変換される
* [x] バイナリ出力（`bytearray` / ファイル書き出し）
  - 完了条件: アセンブル結果を `bytes` として返せる
* [x] エラー報告（行番号付きエラーメッセージ）
  - 完了条件: 不正命令・不正レジスタ・即値範囲外を行番号付きで返す

## 1.3 Build ボタンとアセンブラの接続

* [x] `Build > Build`（F5）でアクティブタブの内容をアセンブルする
  - 完了条件: asm タブがアクティブなとき F5 でビルドが走る
* [x] 成功時にバイナリを `build/out/<node_id>.bin` に保存する
  - 完了条件: `build/out/` に `.bin` ファイルが生成される
* [x] 失敗時に Log パネルにエラーを出す
