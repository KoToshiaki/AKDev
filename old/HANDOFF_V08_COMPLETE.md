...
更新日: 2026-07-24（✅ **v0.8 PHASE COMPLETE** / 次フェーズ v0.9 は計画のみ作成・実装未着手）

---

## ✅ v0.8 PHASE COMPLETE（2026-07-24）— Device Expansion & Connection Validation

### 現在状態

- **ブランチ = `dev`** / **終了前基準コミット `be98d65 docs: expand README in English and Japanese`**。
  STABILIZE のドキュメント更新（`fa58572 docs: finalize v0.8`）と README 拡充（`3d3745f` / `be98d65`）は
  **コミット・push 済み**（`origin/dev` と同期）。
- **v0.8（Device Expansion & Connection Validation）は 14 パッチで実装完了**し、
  `PATCH_V08_STABILIZE_AND_DOCS` で最終整理のうえ、2026-07-24 にユーザーが **`PHASE COMPLETE` を宣言**。
  フェーズ完了処理（`old/` 収納・`ROADMAP9.md`/`CHECKLIST9.md` 作成・本ファイル更新）を実施した。
- **総合テスト（PHASE COMPLETE 処理時に再実行）**: `python -m pytest tests/` → **1182 passed**
  （21 warnings は PySide6 Deprecation のみ・19.9s）。実行前後とも `git status` クリーンで
  `tests/test/system.json` 差分なし。
- **v0.9 は `ROADMAP9.md` / `CHECKLIST9.md` に候補整理のみ**（候補 A〜J・優先順位はユーザー決定待ち）。
  **実装は未着手**。
- **`old/` へ収納したファイル（2026-07-24・`git mv`・削除なし）**:
  `ROADMAP8.md` / `CHECKLIST8.md` / `PATCH_*_V08_*` 全 17 パッチ分 34 ファイル
  （`PATCH_V08_STABILIZE_AND_DOCS_ROADMAP.md` / `..._CHECKLIST.md` を含む）。
- **フェーズ全体の評価**: 目標だった接続検証（warning only）・デバイス拡張・メモリ構成・
  ROM 実行・bitwise 命令・最小ゲーム runtime をすべて達成。既知の制約は GUI 目視未実施のみ。
...
1. **commit / push**（ユーザー操作）— 本 PHASE COMPLETE 整理
   （`old/` 収納 rename + `ROADMAP9.md`/`CHECKLIST9.md` 新規 + `HANDOFF.md`/`README` 更新）。
   推奨メッセージ例: `docs: complete v0.8 phase and prepare v0.9 planning`
2. **v0.9 の優先候補決定**（ユーザー）— `ROADMAP9.md` 候補 A〜J から選択。
   決定後、最初のパッチの `PATCH_*_V09_ROADMAP.md` / `..._CHECKLIST.md` を作成してから実装に着手する。
3. （環境が用意でき次第）**GUI 目視確認** — VRAM Viewer 表示・Run Status 行（v0.8 からの繰越・既知の制約）。
...
- **複数 RAM の真の共存**は 16bit アドレス制約のため未対応（`ROADMAP9.md` 候補 I）。
...
## 📂 ドキュメント整理（2026-06-17 時点の記録 — 当時の現行。現在の現行は `ROADMAP9.md` / `CHECKLIST9.md`）

- **当時の現行管理ファイル = `ROADMAP8.md` / `CHECKLIST8.md`**（2026-07-24 の v0.8 PHASE COMPLETE で `old/` へ収納済み）。
